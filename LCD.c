...
#define LCD_RIGHT 0
	...
	static void convert (uint64_t data,char *Result);
	...
	void LCD_init(void)
	{
		...
		LCD_CONTROL_IO->ddr.allRegister |=(1<<E) | (1<<RW) | (1<<RS);
			...
			LCD_Data_IO->ddr.allRegister =0xFF;
		#else
	...
	{
		uint16_t Fraction = ((uint16_t)(data *100)) %100;

		/* String to hold the ascii result */
		char Result[21];
		 /* 10 for decimal */
		//itoa((int)data,Result,Base);
		convert ((uint64_t)data,Result);
		/* Display The Result */
		LCD_DisplayString(Result);
		if( Fraction != 0 && Base == LCD_DecimalBase)
		{
			//itoa(Fraction,Result,Base);
			convert (Fraction ,Result);
			LCD_DisplayChar('.');
			LCD_DisplayString(Result);
		}
	}
	
	void LCD_DisplayCustomChar(uint8_t Address,uint8_t *LOGO,uint8_t ROW,uint8_t COL)
	{
		Send_instruction(CGRAM_CONST | (Address *8) );
		uint8_t i;
		for(i=0;i<=8;i++)
		{
			
			LCD_DisplayChar(LOGO[i]);
		}
		LCD_Select_RowCol(ROW,COL);
		LCD_DisplayChar(Address);
	}
	
 void convert (uint64_t data,char *Result)
	{
		char X[20];
		uint8_t i=0;
		while(data)
